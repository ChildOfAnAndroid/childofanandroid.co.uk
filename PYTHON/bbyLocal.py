...
def say():
    ...
    except Exception as e: return jsonify(status="error", reply=f"write request failed: {e}"), 500
    ...
    while time.time() - start < TIMEOUT_SECONDS:
        if os.path.exists(resp_path):
            ...
            except Exception as e: reply = f"... (read error: {e})"
    ...
    try:
        ...
        st["speechText"] = reply
        st["isSpeaking"] = True
        with open(STATE_FILE_PATH, "w", encoding="utf-8") as f: json.dump(st, f)
    except Exception:
        pass
