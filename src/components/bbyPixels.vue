...
function cssToPixel(clientX:number, clientY:number){
  const el = overlay.value; if(!el) return null;
  ...
  if (props.isTestCanvas) {
    ...
    px = Math.floor(canvas_x); py = Math.floor(canvas_y);
  } else {
...
function paint(e:PointerEvent){
  ...
  if (isDown && paintedPixelsInStroke.has(pixelKey) && props.mode === 'paint') return;
  ...
  switch (props.mode) {
    ...
    case 'paint': {
      const next = stepColourOnce(strokeState.brushColor, {
        activeEqs: props.activeEQs,
        userColour: props.userSetColor,
        bbyColour: props.bbyColor,
        tempo: props.tempo,
    ...
    case 'erase': {
      const existing = readCurrentRGB(x, y);
...
function updateResolution(res:number){
  ...
  if(props.isTestCanvas){ localPaintData.value = new ImageData(SPRITE_W, SPRITE_H); }
...
if(props.isTestCanvas){
  watch(() => props.resolution, (r) => { if(typeof r === 'number') updateResolution(r); });
}
</script>
