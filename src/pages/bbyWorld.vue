...
function handleCanvasClick(e:MouseEvent){
  ...
  if(!c)return;
  const cell=spatialMap[I(c.x,c.y)];
  if(cell?.alive){selectedCell.value=cell;}else{placeImageAt(c.x,c.y);}
}
...
const updateScope = throttle((e: MouseEvent) => {
  ...
  for (let dy = 0; dy < SS; dy++) {
    for (let dx = 0; dx < SS; dx++) {
      ...
      if (sx >= 0 && sy >= 0 && sx < sz && sy < sz) {
        const o = I(sx, sy) * 4;
        r = frame[o]; g = frame[o + 1]; bl = frame[o + 2]; a = frame[o + 3];
      }
  ...
  if (hx >= 0 && hy >= 0 && hx < sz && hy < sz) {
    ...
    hoverInfo.value = {
      ...
      y: hy,
      heat: heatField[i],
      moisture: moistureField[i],
      nutrient: nutrientField[i],
      solid: solidGrid[i],
      psi: fieldPsi[i],
      lam: fieldLam[i],
      sig: fieldSig[i],
      dyeR: Math.round(dyeRField[i]),
      dyeG: Math.round(dyeGField[i]),
      dyeB: Math.round(dyeBField[i]),
      cell: spatialMap[i]
    };
  }
}, 16);
